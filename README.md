...
- **Trades** — Trading212 REST API (beta API, key-pair auth) or a built-in **paper broker** that simulates fills with spread and FX fees. **Every trade passes an economic-correctness gate first** (ADR 0012): an edge *assumed from the analysts' research* must beat the position's **round-trip** cost (spread + 0.15% FX + platform fee, entry **and** exit, plus 0.5% UK stamp duty on LSE buys) by a configured margin, the order must sit inside `[minOrderValue, min(maxOrderValue, maxOrderValuePct × NAV)]`, leave a net benefit above `minNetBenefitPct`, cover the run's own inference cost, and respect the risk caps (portfolio heat, cash, anti-churn cooldown, conviction).
- **Plan vs funding** — the committee's orders are gated *before* its targets are persisted; a target the run could not fund is kept and marked `UNFUNDED` with the gate's reason, then handed to the next session as a residual (ADR 0013). `GET /api/targets` and the Portfolio page show which targets are actually funded.
- **LLM cost accounting** — every model call's tokens and estimated USD cost are recorded (`llm_usage`), summarised per run in `runs.details.llm`, shown on the Activity page, and bounded by `llm.budget.{maxCallsPerRun,maxSpendPerDayUsd}` (ADR 0011).
...
| `risk` | assumed edge (`baseEdgePct`/`maxEdgePct`), round-trip cost multiplier, net-benefit floor, size window (`minOrderValue`/`maxOrderValue`/`maxOrderValuePct`), inference-cost coverage, heat cap, conviction threshold, cooldown, orders-per-run cap |
| `llm.budget` | max LLM calls per run, max USD spend per trailing window |
